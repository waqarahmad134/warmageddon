        ...
        <div class="col-12">
            <div class="row ">
                <div class="col-md-6 cus_attr">
                    <p>Login And Registration Spin Bonuses</p>
                </div>
                <div class="col-md-6">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#">Login And Registration</a></li>
                            <li class="breadcrumb-item active">Activated Spin Bonuses</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="col-md-12 col-12">
            <div class="card p-4">
                <div class="card-header mb-3">
                    <h4>Add Spin Bonus</h4>
                </div>
                <div class="card-body">
                     @if (@isset($edit))
                     <form action="{{ route('admin_spin_update',$edit->id) }}" method="post"  enctype="multipart/form-data">
                     @else    
                     <form action="{{ route('admin_spin_store') }}" method="post"  enctype="multipart/form-data">
                     @endif
                        @csrf
                        ...
                        <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <select id="status" type="text" class="form-control {{ $errors->has('type') ? ' is-invalid' : '' }}" name="type" required>
                                        <option value="0" {{isset($edit)? $edit->type == 0?'selected':'': ''}}>Registration Spin Bonus</option>
                                        <option value="1" {{isset($edit)? $edit->type == 1 ?'selected':'': ''}}>Login Spin Bonus</option>
                                    </select>
                                    @if ($errors->has('type'))
                                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="table-responsive">
                        <table id="datatables-buttons" class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                  <td>{{ $item->id }}</td>
                                  <td>{{ $item->type == 0? 'Registration Spin': 'Login Spin'}}</td>
                                  <td>{{ $item->title }}</td>
                                  <td>{{ $item->bonus }}</td>
                                  ...
                                  <td>
                                        ...
                                        <a href="{{ route('spin_edit',$item->id) }}" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Edit This"><i class="align-middle" data-feather="edit"></i></a>
                                        <a href="{{ route('spin_change', $item->id )}}" id="active-form-{{ $item->id }}" hidden></a>
                                  </td>
                                </tr>
                                @endforeach
